...
def calibrate_cams_all(config_dict):
    ...
    # Read config_dict
    """
    Taken out because project dir is trial dir and trial dir does not contain calib files
    project_dir = config_dict.get('project').get('project_dir')
    calib_dir = [os.path.join(project_dir, c) for c in os.listdir(project_dir) if ('Calib' in c or 'calib' in c)][0]"""


    session_dir = os.path.realpath(os.path.join(config_dict.get('project').get('project_dir'), '..', '..'))
